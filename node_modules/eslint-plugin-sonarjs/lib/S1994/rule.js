    ...
    create(context) {
        ...
        return {
            ...
            'ForStatement:exit': () => {
                ...
                const hasIntersection = forInfo.testedExpressions.some(testedExpr => forInfo.updatedExpressions.some(updatedExpr => (0, helpers_1.areEquivalent)(updatedExpr, testedExpr, context.sourceCode)));
                if (!hasIntersection) {
                    context.report({
            ...
            'ForStatement MemberExpression': (node) => {
                if (isInsideTest(node) &&
                    (0, helpers_1.getParent)(context, node).type !== 'MemberExpression' &&
                    (0, helpers_1.getParent)(context, node).type !== 'CallExpression') {
                    peekFor().testedExpressions.push(node);
                }
            },
        };
