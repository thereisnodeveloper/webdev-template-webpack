    ...
    create(context) {
        return {
            CallExpression(node) {
                ...
                if ((0, helpers_1.getFullyQualifiedName)(context, callee) === SERVE_STATIC && args.length > 1) {
                    let options = args[1];
                    if (options.type === 'Identifier') {
                        options = (0, helpers_1.getUniqueWriteUsage)(context, options.name, node);
                    }
                    const dotfilesProperty = (0, helpers_1.getProperty)(options, 'dotfiles', context);
